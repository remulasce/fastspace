package com.remulasce.fastspace.input;

import android.graphics.Rect;
import android.opengl.Matrix;

import com.remulasce.fastspace.Vector2;
...
public class Joystick {
	...
	public Mesh	backMesh;	//The static mesh of the background
	public Mesh	topMesh;	//The moving mesh of the interactie part
	...
	public Joystick(int x, int y) {
		...
		backMesh	= ModelManager.makeOverlayRect(BACK_SIZE, BACK_SIZE);
		topMesh		= ModelManager.makeOverlayRect(TOP_SIZE, TOP_SIZE);
	...
	public void setTransformBack(float[] mMatrix) {
		...
		Matrix.translateM(mMatrix, 0, inputArea.left, inputArea.top, 0);
	}
	public void setTransformTop(float[] mMatrix) {
		Matrix.setIdentityM(mMatrix, 0);
		Matrix.translateM(mMatrix, 0,
				inputArea.left+inputArea.width()*((dx+1)/2)-TOP_SIZE/2,
				inputArea.top+inputArea.height()*((-dy+1)/2)-TOP_SIZE/2,
				0);
	}
	...
	public float getTopY() {
		return inputArea.centerY() - inputArea.height() * dy;
	}
	public float getTopX() {
		return inputArea.centerX() - inputArea.width() * dx;
	}
